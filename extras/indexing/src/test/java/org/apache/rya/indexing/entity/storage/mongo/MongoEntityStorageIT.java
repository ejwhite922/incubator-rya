...
public class MongoEntityStorageIT extends MongoITBase {
    ...
    public void search_byDataType() throws Exception {
        ...
        try(final ConvertingCursor<TypedEntity> it = storage.search(icecreamType, new HashSet<>())) {
            while(it.hasNext()) {
                objects.add(it.next());
            }
        }
    ...
    public void search_byFields() throws Exception {
        ...
        try(final ConvertingCursor<TypedEntity> it = storage.search(personType, searchValues)) {
            while(it.hasNext()) {
                objects.add(it.next());
            }
        }
